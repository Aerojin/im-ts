...
export class MessageWrap {
  ...
  public get send(): boolean {
    // return this.message.fromUID;
    return this.message.fromUID === "5ffdd74b21774efeb79631e71919e46a"; // WKApp.loginInfo.uid
  }
  ...
  private get nextIsSamePerson(): boolean {
    if (
      this.nextMessage?.content.contentType === MessageContentTypeConst.time
    ) {
      return false;
    }
    ...
    for (let index = 0; index < len; index++) {
      ...
      if (part.type === PartType.text) {
        ...
        while (text.length > 0) {
          ...
          newParts.push(
            new Part(PartType.emoji, text.substr(index, matchResult[0].length))
          );
