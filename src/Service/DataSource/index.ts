...
export class ChannelDataSource implements IChannelDataSource {
  ...
  async exitChannel(channel: Channel): Promise<void> {
    ...
    return this.WKApp.apiClient.post(`groups/${channel.channelID}/exit`);
  }
  ...
  async channelTransferOwner(channel: Channel, toUID: string): Promise<void> {
    ...
    return this.WKApp.apiClient.post(
  ...
  async subscriberAttrUpdate(
    ...
    return this.WKApp.apiClient.put(
      `groups/${channel.channelID}/members/${subscriberUID}`,
      attr
    );
  }
  createChannel(uids: string[]): Promise<any> {
    return this.WKApp.apiClient.post(`group/create`, { members: uids });
  }
  async groupSaveList(): Promise<ChannelInfo[]> {
    const resp = await this.WKApp.apiClient.get("group/my", {
      param: {
        limit: 100000,
      },
    });
    ...
    if (resp) {
      ...
      for (const data of resp) {
        ...
        channelInfo.logo = this.WKApp.shared.avatarChannel(channelInfo.channel);
  ...
  removeSubscribers(channel: Channel, uids: string[]): Promise<void> {
    return this.WKApp.apiClient.delete(`groups/${channel.channelID}/members`, {
      data: {
        members: uids,
      },
    });
  ...
  addSubscribers(channel: Channel, uids: string[]): Promise<void> {
    return this.WKApp.apiClient.post(`groups/${channel.channelID}/members`, {
      members: uids,
    });
  }
  ...
  updateField(channel: Channel, field: string, value: string): Promise<void> {
    const param: any = {};
    param[field] = value;
    return this.WKApp.apiClient.put(`groups/${channel.channelID}`, param);
  }

  qrcode(channel: Channel): Promise<ChannelQrcodeResp> {
    return this.WKApp.apiClient.get(`groups/${channel.channelID}/qrcode`, {
      resp: () => {
  ...
  async updateSetting(setting: any, channel: Channel): Promise<void> {
    if (channel.channelType === ChannelTypeGroup) {
      ...
      // 个人信息
      return this.WKApp.apiClient.put(
        `users/${channel.channelID}/setting`,
        setting
      );
    }
  }
  ...
  async managerRemove(channel: Channel, uids: string[]): Promise<void> {
    return this.WKApp.apiClient.delete(`groups/${channel.channelID}/managers`, {
      data: uids,
    });
  }
  ...
  async managerAdd(channel: Channel, uids: string[]): Promise<void> {
    return this.WKApp.apiClient.post(
      `groups/${channel.channelID}/managers`,
      uids
    );
  }
  ...
  blacklistAdd(channel: Channel, uids: string[]): Promise<void> {
    return this.WKApp.apiClient.post(
      `groups/${channel.channelID}/blacklist/add`,
      {
        uids: uids,
      }
    );
...
export class CommonDataSource implements ICommonDataSource {
  ...
  requestLoginWithUsernameAndPwd(username: string, password: string) {
  ...
  blacklistRemove(uid: string): Promise<void> {
    return this.WKApp.apiClient.delete(`user/blacklist/${uid}`);
  }
  deleteFriend(uid: string): Promise<void> {
    return this.WKApp.apiClient.delete(`friends/${uid}`);
  }

  userRemark(uid: string, remark: string): Promise<void> {
    return this.WKApp.apiClient.put(`friend/remark`, {
      uid: uid,
      remark: remark,
    });
  }
  getFavoritesAll(): Promise<any> {
    ...
    return this.WKApp.apiClient.get(`favorite/my?page_index=1&page_size=10000`);
  }
  favorities(message: Message): Promise<void> {
    ...
    return this.WKApp.apiClient.post(`favorites`, {
  ...
  favoritiesDelete(id: string): Promise<void> {
    return this.WKApp.apiClient.delete(`favorites/${id}`);
  }
  userStickerCategory(): Promise<any> {
    return this.WKApp.apiClient.get(`sticker/user/category`);
  }
  getStickers(category: string): Promise<any> {
    return this.WKApp.apiClient.get(
      `sticker/user/sticker?category=${category}`
    );
  }
  searchUser(keyword: string): Promise<any> {
    return this.WKApp.apiClient.get(`user/search?keyword=${keyword}`);
  }
  qrcodeMy(): Promise<any> {
    return this.WKApp.apiClient.get("user/qrcode");
  }

  friendSure(token: string): Promise<void> {
    return this.WKApp.apiClient.post("friend/sure", {
      token: token,
    });
  }
  ...
  friendApply(req: {
    ...
    return this.WKApp.apiClient.post(`friend/apply`, {
      to_uid: req.uid,
      remark: req.remark,
      vercode: req.vercode,
    });
  ...
  getImageURL(path: string, opts?: { width: number; height: number }): string {
    ...
    const baseURl = this.WKApp.apiClient.config.apiURL;
    return `${baseURl}${path}`;
  }
  ...
  //     const results = await this.WKApp.apiClient.get(`friend/sync`, {
  ...
  imConnectAddr(): Promise<string> {
    return this.WKApp.apiClient
      .get(`users/${this.WKApp.loginInfo.uid}/im`)
      .then((resp: any) => {
  ...
  async searchFriends(keyword?: string): Promise<ChannelInfo[]> {
    let resp = await this.WKApp.apiClient.get("friend/sync", {
      param: {
        keyword: keyword,
        api_version: "1",
      },
