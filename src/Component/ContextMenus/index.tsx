  ...
  implements ContextMenusContext
{
  ...
  show(event: React.MouseEvent<Element, MouseEvent>): void {
    ...
    const rootW = 80; // this.contextMenusRef.offsetWidth || 0;
    const rootH = 100; // this.contextMenusRef.offsetHeight || 0;

    const showBottom = screenH - clickY <= rootH;
    const showLeft = screenW - clickX <= rootW;

    if (showLeft) {
      this.contextMenusRef.style.left = `${clickX - rootW - left}px`;
    } else {
      this.contextMenusRef.style.left = `${clickX + 5 - left}px`;
    }

    if (showBottom) {
      this.contextMenusRef.style.top = `${clickY - rootH - top}px`;
      this.setState({
        contextOrigin: rootH,
        showContextMenus: true,
      });
    } else {
      this.contextMenusRef.style.top = `${clickY - top}px`;
      this.setState({
        contextOrigin: 0,
        showContextMenus: true,
      });
      ...
      <>
        <div
          className={classNames(
            "wk-contextmenus",
            showContextMenus && "wk-contextmenus-open"
          )}
          ref={(ref) => {
